...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const projectTiles = document.querySelectorAll(".details-container");
  projectTiles.forEach(function (tile) {
    const rect = tile.getBoundingClientRect();
    const skillsetCard = tile.querySelector(".skillset-card");
    if (rect.left < window.innerWidth / 2) {
      skillsetCard.classList.add("right");
    } else {
      skillsetCard.classList.add("left");
    }
  });
});
